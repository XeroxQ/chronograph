...
class Timer {
	constructor(id, name, time, unit, startTime) {
		this._id = id;
		this._name = name;
		this._time = time;
		this._unit = unit;
		...
		// their lifespan.
		this._start(!!existingTimer, startTime);
	}
	...
	stop(replacing) {
		...
		delete(timersActive[this._id]);
		Homey.ManagerSettings.set('timers_active', timersActive);

		if (!replacing) {
			Chronograph.cards.timer_stopped.trigger({
				// Tokens.
				"name": this._name,
				"seconds": Math.round(this.getDuration() / 1e1) / 1e2
			}, {
				// State.
				"timerId": this._id
			}, () => {
	...
	_start(replacing, startTime) {
		this._startTime = startTime || (new Date()).getTime();
		//this._finishReference = setTimeout(this._onFinish.bind(this), this._duration);
		this._finishReference = setTimeout(this._onFinish.bind(this), this.getDuration());
		this._splits = [];
		...
		if (splits instanceof Array) {
			for (let index = 0; index < splits.length; index++) {
				if (splits[index].name == this._name) {
					...
					let delay = this.getDuration() - duration;
					if (delay > 0 ) {
		...
		if (!startTime) {
			Chronograph.cards.timer_started.trigger({
				// Tokens.
				"name": this._name,
				"seconds": Math.round(this._duration / 1e1) / 1e2
			}, {
				// State.
				"timerId": this._id
			}, () => {
		...
		Homey.ManagerApi.realtime(replacing ? 'timer_updated' : 'timer_created', {
			id: this._id,
			name: this._name,
			duration: this.getDuration()
		});

		Chronograph.log("Timer “" + this._name + "” started with duration of " + this.getDuration() + " ms.");
	}
	...
	_onSplit(time, unit) {
		Chronograph.cards.timer_split.trigger({
			// Tokens.
			"name": this._name,
			"seconds": Math.round(Chronograph.calculateDuration(time, unit) / 1e1) / 1e2
		}, {
			// State.
			"timerId": this._id,
			"time": time,
