...
class Stopwatch {
	constructor(id, name, started) {
		...
		this._start(!!existingStopwatch, started);
	}
	...
	getDuration() {
		return Math.max(0.0, (new Date()).getTime() - this._started);
	}

	setDuration(time, unit, absolute) {
		let duration = Chronograph.calculateDuration(time, unit);
		this._started = absolute ? (new Date()).getTime() - duration : this._started + (-duration);

		let splits = [];
		for (var index = 0; index < this._splits.length; index++) {
			clearTimeout(this._splits[index].reference);
			let delay = this._started + this._splits[index].duration - (new Date()).getTime();
			if (delay > 0 ) {
	...
	stop(replacing) {
		...
		delete(stopwatchesActive[this._id]);
		Homey.ManagerSettings.set('stopwatches_active', stopwatchesActive);

		if (!replacing) {
			Chronograph.cards.stopwatch_stopped.trigger({
				// Tokens.
				"name": this._name,
	...
	_start(replacing, started) {
		...
		Homey.ManagerApi.realtime(replacing ? 'stopwatch_updated' : 'stopwatch_created', {
			id: this._id,
			name: this._name,
			duration: this.getDuration()
		});
