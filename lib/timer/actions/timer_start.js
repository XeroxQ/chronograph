...
class TimerStart extends Homey.FlowCardAction {
	constructor(actionId) {
		...
		this.registerRunListener((args) => {
			...
			let timer = new Chronograph(id, args.name, time, args.unit);
			...
			splits
				.filter(split => {
					if (split.name.includes('*')) {
						...
						return split.name == timer.getName();
					}
				})
				.forEach(split => {
	...
	_storeCards() {
		this.getArgumentValues().then(args => {
			Homey.ManagerSettings.set('timer_start_cards', args);
			Chronograph.events.emit('timer_start_cards_updated', args);
		});
	}
}
