...
class Chronograph {
	...
	addSplit(time, unit, data) {
		...
		Chronograph.events.emit('log', this, "Chronograph split added " + time + " // " + unit, LogLevel.INFO);
	}
	...
	static generateId(prefix, name) {
		...
		return Math.abs(hash).toString() + '_' + name.toLowerCase().replace(/[^a-z]/g, '');
	}
}
